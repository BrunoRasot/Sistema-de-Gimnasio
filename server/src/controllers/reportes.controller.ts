...
export const getReporteVentas = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const ventasPorFecha = ventas.reduce((acc: any, v) => {
      const fecha = new Date(v.createdAt).toLocaleDateString('es-PE', { day: '2-digit', month: 'short' });
      acc[fecha] = (acc[fecha] || 0) + Number(v.total);
      return acc;
    }, {});

    const chartData = Object.keys(ventasPorFecha).map(k => ({ fecha: k, ingresos: ventasPorFecha[k] }));
...
export const getReporteMembresias = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    return res.status(500).json({ message: 'Error al generar reporte de membresías' });
  }
};
...
export const getReporteAsistencias = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const picoHora = chartData.length > 0 
      ? chartData.reduce((prev, current) => (prev.ingresos > current.ingresos) ? prev : current).hora 
      : '--:--';

    return res.json({ total: asistencias.length, picoHora, chartData });
  } catch (error) {
    return res.status(500).json({ message: 'Error al generar reporte de asistencias' });
  }
};
