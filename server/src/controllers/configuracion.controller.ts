...
export const actualizarInfo = async (req: Request, res: Response): Promise<any> => {
    try {
        const { nombre, ruc, telefono, email, direccion, zonaHoraria, moneda, logo } = req.body;
        await obtenerConfig();

        const configActualizada = await prisma.configuracion.update({
            where: { id: 1 },
            data: { nombre, ruc, telefono, email, direccion, zonaHoraria, moneda, logo }
        });
...
export const actualizarNotificaciones = async (req: Request, res: Response): Promise<any> => {
    try {
        const { emailNotificaciones, nuevasVentas, membresiasVencidas, stockBajo, alertasSistema, reportesSemanales } = req.body;
        await obtenerConfig();
        const configActualizada = await prisma.configuracion.update({
            where: { id: 1 },
            data: { emailNotificaciones, nuevasVentas, membresiasVencidas, stockBajo, alertasSistema, reportesSemanales }
        });
