    ...
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto space-y-4 text-gray-900">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900 tracking-wide">Membresías Vencidas</h1>
            <p className="text-[11px] text-gray-500 mt-0.5">Clientes con planes caducados. Contáctalos para renovar.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3.5 shadow-sm">
          <div className="p-2.5 rounded-lg bg-red-50 border border-red-100"><AlertTriangle className="w-4 h-4 text-red-500" /></div>
          <div>
            <p className="text-xl font-bold text-gray-900 leading-none">{miembrosVencidos.length}</p>
            <p className="text-[11px] text-gray-500 mt-1">Clientes Vencidos</p>
          </div>
        </div>
        <div className="md:col-span-2 relative flex items-center">
          ...
          <input
            ...
            className="w-full h-full bg-white border border-gray-200 rounded-xl py-3 pl-10 pr-4 text-xs text-gray-900 focus:border-red-400 focus:ring-1 focus:ring-red-400 outline-none transition-all placeholder-gray-400 shadow-sm"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-500 border-b border-gray-200">
                ...
                <th className="py-3 px-4 text-center">Tiempo Vencido</th>
            ...
            <tbody className="divide-y divide-gray-100 text-xs">
              {cargando ? (
                ...
                <tr>
                  <td colSpan={5} className="py-14 text-center text-gray-500">
                    No hay membresías vencidas registradas.
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={m.id} className="hover:bg-red-50/30 transition-colors">
                      <td className="py-3 px-4 pl-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-red-50 flex items-center justify-center border border-red-100 font-bold text-red-600 text-[10px] shrink-0">
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="inline-flex px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-md bg-red-50 text-red-600 border border-red-200">
                          Hace {Math.abs(diasRestantes)} días
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center pr-5">
                        <button onClick={() => handleAbrirRenovacion(m)} className="px-3 py-1.5 bg-gray-900 hover:bg-black text-white text-xs font-medium rounded-lg transition-all shadow-sm">
                          Renovar
                        </button>
                      </td>
                    </tr>
