...
import { Search, Loader2, AlertTriangle, Tag, Save, X, CalendarCheck } from 'lucide-react';
...
export default function MembresiasVencidasPage() {
  ...
  const miembrosVencidos = useMemo(() => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto text-gray-900">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 bg-white p-6 rounded-2xl border border-red-100 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-red-50 rounded-xl border border-red-100 shadow-sm">
            <AlertTriangle className="w-6 h-6 text-red-500" />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Membresías Vencidas</h1>
            <p className="text-sm text-gray-500 mt-0.5">Clientes con planes caducados. Contáctalos para renovar.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center gap-4 shadow-sm">
          <div className="p-3 rounded-xl bg-red-50 border border-red-100"><AlertTriangle className="w-5 h-5 text-red-500" /></div>
          <div>
            <p className="text-2xl font-bold text-gray-900 leading-none">{miembrosVencidos.length}</p>
            <p className="text-xs text-gray-500 mt-1">Clientes Vencidos</p>
          </div>
        </div>
        <div className="md:col-span-2 relative">
          ...
          <input
            ...
            className="w-full h-full bg-white border border-gray-200 rounded-2xl py-3 pl-11 pr-4 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 transition-all shadow-sm"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-gray-50 text-[10px] font-bold uppercase tracking-widest text-gray-500 border-b border-gray-200">
                ...
                <th className="p-5 text-center">Tiempo Vencido</th>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              ...
              ) : miembrosVencidos.length === 0 ? (
                <tr><td colSpan={5} className="p-12 text-center text-gray-500">No hay membresías vencidas registradas.</td></tr>
              ) : (
                    ...
                    <tr key={m.id} className="hover:bg-red-50/30 transition-colors">
                      <td className="p-4 pl-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center border border-red-100 font-bold text-red-600 text-xs shrink-0">
                      ...
                      <td className="p-4 text-center">
                        <span className="inline-flex px-3 py-1 text-[11px] font-bold rounded-full bg-red-50 text-red-600 border border-red-200">
                          Hace {Math.abs(diasRestantes)} días
                        </span>
                      </td>
                      <td className="p-4 text-center">
                        <button onClick={() => handleAbrirRenovacion(m)} className="px-4 py-2 bg-gray-900 hover:bg-black text-white text-xs font-bold rounded-lg transition-all shadow-sm">
                          Renovar
                        </button>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
              <div>
                <h2 className="text-xl font-extrabold text-gray-900">Renovar Membresía</h2>
