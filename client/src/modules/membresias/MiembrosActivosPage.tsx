...
export default function MiembrosActivosPage() {
  ...
  const handleBuscarDni = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.mensaje || 'No se pudo encontrar al cliente.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.mensaje || 'No se pudo asignar la membresía.');
    } finally {
      setGuardando(false);
    }
  };
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">
              Clientes con planes de membresía vigentes.
            </p>
          </div>
        </div>
        <button
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-center">
                        ...
                        <p
                          className={`text-[10px] font-semibold mt-0.5 ${diasRestantes <= 5 ? 'text-red-500' : 'text-gray-500'}`}
                        >
          ...
          <div className="w-full max-w-lg bg-white border border-gray-200 rounded-xl shadow-xl flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/50">
              <div>
                ...
                <p className="text-[11px] text-gray-500">
                  Busca el DNI del cliente y asígnale su membresía.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                {clienteEncontrado && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3 mt-2">
                    <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-green-200 font-bold text-green-700 text-[10px] shrink-0">
                      {clienteEncontrado.nombres.charAt(0)}
                      {clienteEncontrado.apellidos.charAt(0)}
                    </div>
                    <div>
                      ...
                      <p className="text-[10px] text-green-700">
                        ...
                        {clienteEncontrado.telefono || 'Sin teléfono'}
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs outline-none focus:border-gray-900 font-medium"
                      >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-gray-200 rounded-lg py-1.5 pl-8 pr-3 text-xs outline-none focus:border-gray-900"
                        />
                      </div>
                    </div>
                  </div>
              ...
              <div className="pt-3 flex justify-end gap-2 border-t border-gray-100 mt-4">
                ...
                <button
                  type="submit"
                  disabled={guardando || !clienteEncontrado || planes.length === 0}
