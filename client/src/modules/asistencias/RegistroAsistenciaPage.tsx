...
export default function RegistroAsistenciaPage() {
  ...
  const handleRegistrar = async () => {
    ...
    setRegistrando(true);
    try {
      await registrarAsistencia(miembro.id);
    ...
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Busque un cliente, valide su membresía y registre su acceso.</p>
          </div>
        </div>
      </div>

      <form onSubmit={handleBuscar} className="bg-white border border-gray-200 rounded-xl px-4 py-2.5 flex items-center gap-3 shadow-sm">
      ...
      {miembro && (
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className={`p-6 border-b ${tieneAcceso ? 'bg-green-50/30 border-green-50' : 'bg-red-50/30 border-red-50'}`}>
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{miembro.nombres} {miembro.apellidos}</h2>
          ...
          <div className="p-6 space-y-6">
            <div>
              <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Detalles de la Membresía</h3>
              {tieneAcceso ? (
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
                    <p className="text-xs text-gray-500 mb-1">Plan Contratado</p>
                    <p className="text-sm font-bold text-gray-900">{membresiaActiva.plan?.nombre || 'Plan Estándar'}</p>
                  </div>
                  <div className="p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
              <button
                onClick={() => { setMiembro(null); setDni(''); }}
